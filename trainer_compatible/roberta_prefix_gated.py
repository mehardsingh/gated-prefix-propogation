...
class RobertaEncoder_Prefix_Gated(RobertaEncoder):
    ...
    def add_curr_prefix(self, layer_idx, curr_hidden_state):
        ...
        existing_prefix = curr_hidden_state[:,:self.config.prefix_len,:]
        next_prefix = self.prefix[layer_idx].unsqueeze(0).repeat(batch_size, 1, 1)

        if layer_idx == 0:
            new_prefix = next_prefix
        else:
            concat_prefix = torch.cat([existing_prefix, next_prefix], dim=1)
            pooled_prefix = torch.mean(concat_prefix, dim=1) 
            gate_mlp = self.gate_mlps[layer_idx]
            gate = gate_mlp(pooled_prefix).unsqueeze(-1)
...
class RobertaModel_Prefix_Gated(RobertaModel):
    def __init__(self, config):
        super().__init__(config)
        self.encoder = RobertaEncoder_Prefix_Gated(config)
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0][:,self.config.prefix_len:,:]
...
class RobertaForSequenceClassification_Prefix_Gated(RobertaForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
        self.roberta = RobertaModel_Prefix_Gated(config)
...
# model = RobertaForSequenceClassification_Prefix_Gated(config=config)
